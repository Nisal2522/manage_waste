...
// QR Code and Collection APIs
export const getBinByQRCode = async (qrCode) => {
  const response = await api.get(`/bins/qr/${encodeURIComponent(qrCode)}`);
  return response.data;
};

export const markBinAsCollected = async (collectionData) => {
  const response = await api.post('/bins/collect', collectionData);
  return response.data;
};
